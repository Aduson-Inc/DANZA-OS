...
(`danzaboss/` — kernel, CORTEX, hooks, research, runtime; 339 tests).
...
danzaboss/                  THE BRAIN (promoted, authoritative) — Python, stdlib only, 339 tests
  ...
  tests/  run_tests.sh       339 unit tests + cold-start harness
...
- **Run the test suite:** `./danzaboss/run_tests.sh` (339 tests)
- **CORTEX memory:** `PYTHONPATH=. python3 -m danzaboss.cli cortex <search|get|observe|retrieve|context|age|learn|stats|ui>` —

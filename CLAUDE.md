...
(`danzaboss/` — kernel, CORTEX, hooks, research, runtime; 311 tests).
...
danzaboss/                  THE BRAIN (promoted, authoritative) — Python, stdlib only, 311 tests
  ...
  tests/  run_tests.sh       311 unit tests + cold-start harness
...
- **Run the test suite:** `./danzaboss/run_tests.sh` (311 tests)
- **CORTEX memory:** `PYTHONPATH=. python3 -m danzaboss.cli cortex <search|get|observe|retrieve|context|stats|ui>` —

...
carmella-researcher --task "<your assigned task>"` yourself. Treat the
returned package as your primary task knowledge — a role-budgeted slice of
what the team already knows — even when Tony-D already pasted a
`## CORTEX Context` block into your spawn prompt. Do not re-explore the whole
repo for facts the package already gives you. Pull more only with

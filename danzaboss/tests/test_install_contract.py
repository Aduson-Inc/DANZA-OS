...
class InstallerContract(unittest.TestCase):
    def test_posix_wrapper_is_strict_and_pins_test_branch(self):
        script = ROOT / "install.sh"
        text = script.read_text(encoding="utf-8")
        self.assertEqual(subprocess.run(["bash", "-n", str(script)]).returncode, 0)
        self.assertIn("set -euo pipefail", text)
        self.assertIn("codex/production-danzaboss-install-flow", text)
        self.assertNotIn("@main", text)
        self.assertIn("approve", text.lower())

    def test_windows_wrapper_is_present_and_pins_test_branch(self):

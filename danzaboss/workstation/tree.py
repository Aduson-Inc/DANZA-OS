...
FLOW: tuple[Step, ...] = (
    ...
    Step("p_seed", "phase", "Your idea", (
        Question("seed_name", "What should we call this idea?", "text"),
        Question("seed_intent", "Tell us about it in a few sentences",
                 "longtext"),
        Question("seed_links", "Any links or references?", "list",
                 required=False),
    )),
    Step("p1", "phase", "Concept", (
        Question("project_name", "What should we call the project?", "text"),
        ...
        Question("concept_similar", "Are there similar products you like?",
                 "list", required=False),
        Question("features_must", "Which features must it have?", "list"),
        Question("features_nice", "Any nice-to-have features?", "list",
                 required=False),
    ...
    Step("p2", "phase", "Features", (
        Question("capabilities", "Which of these will it need?", "multi",
                 options=("accounts_auth", "payments", "admin_panel",
                          "notifications", "file_uploads", "search",
                          "realtime"),
                 required=False),
        Question("feature_notes", "Anything else about these features?",
                 "longtext", required=False),
    )),
    Step("p3", "phase", "Stack", (
        Question("stack_choice", "How should we choose the tech?", "choice",
                 options=("template", "custom", "no_preference")),
        Question("stack_template", "Which template would you like?", "text",
                 show_if=(("stack_choice", ("template",)),)),
        Question("stack_custom", "Describe the tech you want to use",
                 "longtext", show_if=(("stack_choice", ("custom",)),)),
    )),
    Step("cp_stack", "checkpoint", "Stack review"),
    Step("p4", "phase", "Design", (
        Question("design_urls", "Links to designs you like", "list",
                 required=False),
        Question("design_uploads", "Upload images you like", "uploads",
                 required=False),
        Question("color_direction", "How should colors be chosen?", "choice",
                 options=("pick", "upload", "propose"), default="propose"),
        Question("style_words", "Style words (minimal, playful, ...)",
                 "list", required=False),
        Question("design_notes", "Anything else about the look?", "longtext",
                 required=False),
    )),
    Step("p5", "phase", "Practicalities", (
        Question("repo_mode", "Start brand new, or build onto existing code?",
                 "choice", options=("fresh", "existing")),
        Question("deploy_intent", "Where will it run?", "choice",
                 options=("local", "vps", "platform"), default="local"),
        Question("cadence", "How should the team run the build?", "choice",
